#ifndef VULKANFS_ENTRY_HPP
#define VULKANFS_ENTRY_HPP
...
namespace vulkanfs {
    namespace entry {
        ...
        private:
            ...
            std::map<off_t, vulkanfs::block_ref> file_blocks;

            // Last block touched by write()
            vulkanfs::block_ref last_written_block;
            ...
            // Get the GPU buffer of the block if it exists or a nullptr
            vulkanfs::block_ref get_block(off_t off) const;

            // Allocate new block, buf is only set on success (returning true)
            vulkanfs::block_ref alloc_block(off_t off);
